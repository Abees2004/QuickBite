    ...
    const handleAction = async (newStatus) => {
        try {
            await axiosInstance.patch(
                `/api/register/partner-approve/${user.id}/`,
                { status: newStatus }
            );
                ...
                <div className="p-4 sm:p-6">
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        <p className="text-sm flex justify-between">
                            ...
                            <span className="text-gray-900 font-medium">{user.partner.username}</span>
                        </p>
                        <p className="text-sm flex justify-between">
                            <span className="font-semibold text-gray-600">Email:</span>
                            <span className="text-primary truncate ml-2 font-medium">{user.partner.email}</span>
                        </p>
                        <p className="text-sm flex justify-between">
                            <span className="font-semibold text-gray-600">Resturant Name:</span>
                            <span className="text-primary truncate ml-2 font-medium">{user.restaurant_name}</span>
                        </p>
                        <p className="text-sm flex justify-between">
                            <span className="font-semibold text-gray-600">Location:</span>
                            <span className="text-primary truncate ml-2 font-medium">{user.city}</span>
                        </p>
                        <p className="text-sm flex justify-between">
