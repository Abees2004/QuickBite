                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Resturant Name</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {users.map((user) => (
                            <tr key={user.id} className="hover:bg-gray-50 transition">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{user.partner.username}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{user.restaurant_name}</td>
                    ...
                    <div key={user.id} className="bg-white p-4 shadow rounded-lg border border-gray-200">
                        ...
                        <div className="flex justify-between items-center pt-3 border-t">
                            ...
                            <div className="space-x-3">
                                <button onClick={() => handleApiAction(user, 'A')} className="text-green-600 text-sm font-bold">Approve</button>
                                <button onClick={() => handleApiAction(user, 'R')} className="text-red-600 text-sm font-bold">Reject</button>
                            </div>
                        </div>
                    </div>
                ))}
